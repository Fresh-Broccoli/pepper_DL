...
class YoloManager():
    ...
    def predict(self, frame, augment=False, conf_thres=0.25, preprocess = True, classes=None, iou_thres=0.45, agnostic_nms=False):
        if preprocess:
...
if __name__ == "__main__":
    ...
    img = cv2.imread(os.path.join("data", "custom", "person.jpg"))

    preprocessed_img = manager.preprocess_frame(img)
    pred = manager.predict(img, conf_thres=0.50)
